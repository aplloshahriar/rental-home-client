...
const DetailsHomes = () => {
  ...
  const handleBooking = () => {
    ...
    const arrivalMoment = moment.utc(arrivalDate).local(); // Convert to local time zone
    const departureMoment = moment.utc(departureDate).local(); // Convert to local time zone
    ...
    while (currentDate.isSameOrBefore(departureMoment)) {
      // Format the current date and push it to the bookingDates array
